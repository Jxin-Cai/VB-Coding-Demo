---
description: 基于可用的设计文档，为功能生成可操作的、按依赖顺序排列的 tasks.md
handoffs: 
  - label: 分析一致性
...
1. **设置**: 从代码库根目录运行 `.specify/scripts/bash/check-prerequisites.sh --json`，解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于包含单引号的参数（如 "I'm Groot"），使用转义语法：例如 'I'\''m Groot'（或者如果可能使用双引号："I'm Groot"）。
