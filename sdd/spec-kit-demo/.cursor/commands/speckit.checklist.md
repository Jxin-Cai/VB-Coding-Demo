...
**隐喻**：如果您的规格是用英语编写的代码，清单就是它的单元测试套件。您正在测试需求是否编写良好、完整、明确且准备好实施 - 而不是实施是否有效。
...
1. **设置**：从代码库根目录运行 `.specify/scripts/bash/check-prerequisites.sh --json`，解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表的 JSON 输出。
   - 所有文件路径必须是绝对路径。
...
2. **澄清意图（动态）**：推导最多三个初始上下文澄清问题（无预制目录）。它们必须：
   ...
   生成算法：
   1. 提取信号：功能领域关键词（例如，auth、latency、UX、API）、风险指标（"critical"、"must"、"compliance"）、利益相关者提示（"QA"、"review"、"security team"）和明确的交付成果（"a11y"、"rollback"、"contracts"）。
   2. 将信号聚类为候选焦点领域（最多 4 个），按相关性排名。
   ...
   输出问题（标记 Q1/Q2/Q3）。回答后：如果 ≥2 个场景类（Alternate / Exception / Recovery / Non-Functional domain）仍不清楚，您可以提出最多两个针对性的后续问题（Q4/Q5），每个都有一行理由（例如，"未解决的恢复路径风险"）。不要超过五个总问题。如果用户明确拒绝更多，则跳过升级。
...
5. **生成清单** - 创建"需求的单元测试"：
   ...
   - 生成唯一的清单文件名：
     ...
     - 如果文件存在，追加到现有文件
   - 从 CHK001 开始顺序编号项目
   - 每次 `/speckit.checklist` 运行创建一个新文件（永远不会覆盖现有清单）
   ...
   **场景分类和覆盖**（需求质量焦点）：
   - 检查是否存在以下需求：Primary、Alternate、Exception/Error、Recovery、Non-Functional 场景
   - 对于每个场景类，询问："[场景类型]需求是否完整、清晰且一致？"
   - 如果场景类缺失："[场景类型]需求是否有意排除或缺失？[Gap]"
   ...
   **发现和解决问题**（需求质量问题）：
   ...
   - 假设："是否验证了'始终可用的播客 API'的假设？[假设]"
   - 依赖项："是否记录了外部播客 API 需求？[依赖项，Gap]"
   - 缺失定义："是否用可衡量标准定义了'视觉层次'？[Gap]"

   **内容整合**：
   - 软上限：如果原始候选项目 > 40，按风险/影响优先考虑
   - 合并检查相同需求方面的近似重复项
...
6. **结构参考**：遵循 `.specify/templates/checklist-template.md` 中的规范模板生成清单，用于标题、元部分、类别标题和 ID 格式。如果模板不可用，使用：H1 标题，purpose/created 元行，包含 `- [ ] CHK### <需求项目>` 行的 `##` 类别部分，全局递增 ID 从 CHK001 开始。
...
**重要提示**：每次 `/speckit.checklist` 命令调用使用简短、描述性名称创建清单文件，除非文件已存在。这允许：
